...
module.exports = class DisableCmd extends BaseCommand {
	constructor(client) {
		super(client, {
			name: "disablecmd",
			group: "permissions",
			aliases: ["disablecommand", "dcmd"],
			...
			args: [
				...
				{
					key: "type",
					prompt: "Type of target to disable the command for",
					type: "string",
	...
	resolveTarget(ctx, argTypeID) {
		...
		if (value && value.id)
			return { type: argTypeID, id: value.id, enabled: false };
		return "";
	}
	...
	async task(ctx) {
		...
		if (["server", "guild"].includes(ctx.args.type))
			newPerms = { type: "guild", id: ctx.guild.id, enabled: false };
